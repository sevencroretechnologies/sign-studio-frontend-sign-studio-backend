...
export default function Login() {
  ...
  const validateForm = (): boolean => {
    ...
    if (!email.trim()) {
      errors.email = 'Email, Username, or Phone Number is required';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.response?.data?.errors) {
        ...
        if(errors.phone_number) apiErrors.email = errors.phone_number[0];
      ...
      <form onSubmit={handleSubmit}>
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="email" className={fieldErrors.email ? 'text-red-500' : ''}>Email, Username, or Phone</Label>
            <Input
              id="email"
              type="text"
