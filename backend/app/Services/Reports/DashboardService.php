...
class DashboardService
{
    ...
    public function getDashboardData(): array
    {
        return [
            ...
            'attendance' => [
                'today' => $this->attendanceService->getTodaySummary(),
                'weekly_trend' => $this->getWeeklyAttendanceTrend(),
            ],
            'department_split' => $this->getDepartmentDistribution(),
    ...
    public function getPayrollStats(): array
    {
        ...
        return [
            'generated' => $monthlySlips->where('status', 'generated')->count(),
            'paid' => $monthlySlips->where('status', 'paid')->count(),
            'monthly_expense' => $this->getPayrollMonthlyExpenses(),
        ];
    }

    /**
     * Get payroll monthly expenses for the last 6 months.
     */
    public function getPayrollMonthlyExpenses(int $months = 6): array
    {
        $data = [];
        for ($i = $months - 1; $i >= 0; $i--) {
            $date = now()->subMonths($i);
            $period = $date->format('Y-m');
            $amount = SalarySlip::where('salary_period', $period)
                ->whereIn('status', ['paid', 'generated'])
                ->sum('net_payable');
    ...
    public function getWeeklyAttendanceTrend(int $days = 7): array
    {
        ...
        for ($i = $days - 1; $i >= 0; $i--) {
            ...
            // For future dates or today, we might not have all data, but we query anyway
            $presentCount = WorkLog::whereDate('log_date', $date->toDateString())
                ->whereNotNull('clock_in')
                ->count();
            
            // Absent could be calculated as Total - Present (simplified for dashboard trend)
            // Or count specific absent statuses if implemented
            $absentCount = WorkLog::whereDate('log_date', $date->toDateString())
                ->where('status', 'absent')
                ->count();

            // If no explicit absent records, approximate
            if ($absentCount == 0 && $date->isPast() && !$date->isToday()) {
    ...
    public function getEmployeeGrowthTrend(): array
    {
        ...
        for ($i = 11; $i >= 0; $i--) {
            $date = now()->subMonths($i);
            $count = StaffMember::whereDate('date_of_joining', '<=', $date->endOfMonth())
        ...
        return StaffMember::active()
            ...
            ->map(function ($item) use ($totalActive) {
                return [
                    ...
                    'count' => $item->count,
                    'percentage' => round(($item->count / $totalActive) * 100),
                ];
            })
            ->toArray();
    }
