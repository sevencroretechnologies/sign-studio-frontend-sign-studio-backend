...
class CandidateController extends Controller
{
    ...
    {
        $query = Candidate::with(['candidateSource'])->withCount('applications');

        if ($request->status) {
            $query->where('status', $request->status);
        }
        if ($request->candidate_source_id) {
            $query->where('candidate_source_id', $request->candidate_source_id);
        }
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'candidate_source_id' => 'nullable|exists:candidate_sources,id',
        ]);
