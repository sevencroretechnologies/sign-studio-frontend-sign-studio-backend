...
class SalarySlipController extends Controller
{
    ...
    public function calculate(Request $request): JsonResponse
    {
        try {
            ...
            $breakdown = $this->service->calculateSalaryDetails(
                $validated['staff_member_id'],
                $validated['month'],
                $validated['year']
            );

            return $this->success($breakdown, 'Salary calculation preview retrieved successfully');
        } catch (\Exception $e) {
            return $this->serverError('Failed to calculate salary: ' . $e->getMessage());
        }
    }
    ...
    /**
     * Get authenticated user's salary slips.
     * This endpoint is for regular staff to view their own salary slips.
     */
    public function mySlips(Request $request): JsonResponse
    {
        try {
            ...
            if (!$userId) {
                return $this->notFound('User not authenticated. Please log in to continue.');
            }

            // Find the staff member record associated with this user_id
            $staffMember = \App\Models\StaffMember::where('user_id', $userId)->first();

            if (!$staffMember) {
                return $this->notFound('Staff member profile not found for your account. Please contact HR to set up your profile.');
            }
            ...
            return $this->success($result, 'Your salary slips retrieved successfully');
        } catch (\Illuminate\Database\Eloquent\ModelNotFoundException $e) {
            return $this->notFound('Staff member or salary slips not found');
        } catch (\InvalidArgumentException $e) {
            return $this->validationError(['error' => $e->getMessage()]);
        } catch (\Exception $e) {
            \Log::error('Failed to retrieve salary slips for staff member: ' . $e->getMessage(), [
                'user_id' => auth()->id(),
                'trace' => $e->getTraceAsString(),
            ]);
            return $this->serverError('An unexpected error occurred while retrieving your salary slips. Please try again later.');
        }
    }
