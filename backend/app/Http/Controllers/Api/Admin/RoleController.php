...
class RoleController extends Controller
{
    ...
    {
        // Get the authenticated user's role hierarchy level
        $currentUser = auth()->user();
        $currentUserRole = $currentUser->roles->sortBy('hierarchy_level')->first();
        $currentUserHierarchy = $currentUserRole->hierarchy_level ?? 99;

        $query = Role::query()
            ->withCount(['permissions']);

        // Filter roles: only show roles with equal or higher hierarchy level
        // (higher number = lower privilege in the system)
        // Admin (1) can see all roles
        // Org (2) can see roles with hierarchy >= 2
        // Company (3) can see roles with hierarchy >= 3
        // HR (4) can see roles with hierarchy >= 4
        // User (5) can only see user role
        $query->where('hierarchy_level', '>=', $currentUserHierarchy);
